...
constexpr void __advance(InputIter& it,
...
constexpr void __advance(BiDirIter& it,
...
constexpr void __advance(RandIter& it,
...
template <class InputIter, class Distance,
          class IntegralDistance =
              decltype(__convert_to_integral(declval<Distance>())),
          enable_if_t<is_integral<IntegralDistance>::value, int> = 0>
void advance(InputIter& __i, Distance __orig_n) {
    typedef typename iterator_traits<InputIter>::difference_type diff;
    typedef typename iterator_traits<InputIter>::iterator_category cat;
    diff __n = static_cast<diff>(__convert_to_integral(__orig_n));
    static_assert(is_convertible_v<cat, bidirectional_iterator_tag> || __n >= 0,
                  "std::advance: Can only pass a negative `n` with a "
                  "bidirectional_iterator.");
    jstl::__advance(__i, __n, cat());
}
