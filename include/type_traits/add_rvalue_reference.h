#ifndef _JSTL_TYPE_TRAITS_ADD_RVALUE_REFERENCE_H
#define _JSTL_TYPE_TRAITS_ADD_RVALUE_REFERENCE_H

#include "type_traits/is_referenceable.h"

namespace jstl {

namespace detail {

template <class Tp, bool = is_referenceable<Tp>::value>
struct _add_rvalue_reference_impl {
    typedef Tp type;
};

template <class Tp>
struct _add_rvalue_reference_impl<Tp, true> {
    typedef Tp&& type;
};

}  // namespace detail

template <class Tp>
struct add_rvalue_reference {
    using type = typename detail::_add_rvalue_reference_impl<Tp>::type;
};

template <class Tp>
using add_rvalue_reference_t =
    typename detail::_add_rvalue_reference_impl<Tp>::type;

}  // namespace jstl

#endif